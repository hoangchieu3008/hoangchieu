            ...
            <div id='admin_chat_box'>
                <div class='wplc_on_premise_chat_box_user_info'>
                    <div class='wplc_on_premise_chat_box_user_info_avatar'>
                        <img src="//www.gravatar.com/avatar/<?= md5( $session->email ) ?>?d=<?=( isset( $_SERVER['HTTPS'] ) && $_SERVER['HTTPS'] ) ? 'https' : 'http'?>://ui-avatars.com/api//<?=esc_html($session->name)?>/64/<?=TCXUtilsHelper::wplc_color_by_string($session->name)?>/fff" class="admin_chat_img"
                             width="50px"/>
                    </div>
                    <div class='wplc_on_premise_chat_box_user_info_details'><?= esc_html( $session->name ) ?>
                        ...
                        <div class='admin_visitor_advanced_info admin_agent_rating'>
                            <div class='rating <?= $rating_class ?>'><?= $session->getRatingName() ?></div>
                        </div>
                    </div>
							...
							if ( $message->from == "System notification" ) {
                                ...
                                <strong><?= esc_html($message->from) ?>
                                    ( <?= date( 'H:i:s', current_time( strtotime( $message->timestamp ) ) ) ?>
                                        ...
                                        class="history_chat_message"><?= $message->get_message() ?></span>
							<?php }
							?>
                            <br/>
						<?php }
